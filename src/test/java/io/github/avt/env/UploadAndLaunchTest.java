...
import static io.github.avt.env.TestLauncher.TEST_FILE_NAME;
...
public class UploadAndLaunchTest {
  ...
  public void uploadAndRun(TestContext testContext) {
    ...
    vertx.deployVerticle(new AVTService(), deployed -> {
      ...
      startTestDirs.set(vertx.fileSystem().readDirBlocking(AVTService.AVT_HOME_DIR));
      infectionClient.infect(
        new HostWithEnvironment(Commons.LOCALHOST, AVTService.DEFAULT_PORT),
        ...
        .setHandler(ar -> {
          if (ar.succeeded()) {
            ...
            vertx.setPeriodic(100, timerId -> {
              List<String> currentFiles = vertx.fileSystem().readDirBlocking(AVTService.AVT_HOME_DIR);
    ...
    vertx.setTimer(10_0000, event -> {
      List<String> currentDirs = vertx.fileSystem().readDirBlocking(AVTService.AVT_HOME_DIR);
